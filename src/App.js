...
class App extends Component {
  ...
  componentWillMount() {
    const movieColl = JSON.parse(
      window.localStorage.getItem("movie-collection") || '{ "movies": [] }'
    );
    this.setState({
  ...
  submitNewMovie = movie => {
    const updateMovie = {
      [Object.keys(this.state.movies).length + 1]: movie
    };
      ...
      Object.keys(this.state.movies).forEach(movieId => {
        ...
        movies.push(
          ...
          >
            ...
            <Flexbox>Year: {movie.year}</Flexbox>
    ...
    return (
      <Flexbox flexDirection="column" minHeight="100vh" className="App">
        ...
        <Flexbox
          className="overlay"
          style={{
            display: this.state.showAddMovie === true ? "flex" : "none"
          }}
        >
          <Flexbox className="add-movie-container">
            <AddMovie
              onSubmit={this.submitNewMovie}
              onCancel={() =>
                this.setState({
                  showAddMovie: false
                })}
